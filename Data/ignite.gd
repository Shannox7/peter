...
func stats():
	...
	var stats = [["Damage", damaged], ["Burn Time", burn_timed]]
	return stats
...
#		set_fixed_process(true)
		ignite = true
		fire = effect.get_node("Fire").duplicate()
		get_parent().add_child(fire)
		fire.set_global_pos(get_global_pos())
		fire.get_node("Area2D").set_scale(Vector2(get_scale().x, 1))
		fire.get_node("fire").set_emission_half_extents(Vector2(get_scale().x * fire.get_node("fire").get_emission_half_extents().x, fire.get_node("fire").get_emission_half_extents().y))
		...
		fire.get_node("fire").set_amount(fire.get_node("fire").get_amount() * get_scale().x)

func consume(collider):
#	if !weakref(collider).get_ref():
	if collider.get_parent().consume == false:
	...
	if not ignite and not consume:
		if get_node("RayCast2D").is_colliding():
			if get_node("RayCast2D").get_collider() != null:
				if get_node("RayCast2D").get_collider().is_in_group("oil"):
					consume = true
