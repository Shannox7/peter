...
#	get_node("dzones").move_child(defence, get_node("dzones").get_child_count() - 1)
...
func win(win_faction, lose_faction, attack):
	for players in win_faction.player_list:
		if not players.AI:
			...
			defending_land.AI = false
			defending_land.controller = players.myself
			defending_land.players.pop_front()  
			defending_land.players.append(players)
